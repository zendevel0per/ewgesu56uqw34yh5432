...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata, phone)
    ---@class xPlayer
    local self = {}
    ...
    self.variables = {}
    self.weight = weight
    self.maxWeight = Config.MaxWeight * 1000
...
--
    ...
    function self.addInventoryItem(itemName, count)
        ...
        if item then
            ...
            local baseType = Config.Base.BaseType
            if baseType ~= 'LIMIT' then
                self.weight = (self.weight or 0) + ((item.weight or 0) * count)
            end
...
--
    ...
    function self.removeInventoryItem(itemName, count)
        ...
        if item then
            count = ESX.Math.Round(count)
            if count <= 0 then
                error(("Player ID:^5%s Tried to remove an invalid count -> %s of %s"):format(self.playerId, count, itemName))
                return
            end
...
--
    ...
    function self.canCarryItem(itemName, count)
        ...
        if baseType == 'LIMIT' then
            ...
            return (currentCount + count) <= item.limit
    
        elseif baseType == 'WEIGHT' or baseType == '2Type' then
    ...
    ---@param ammo number
    ---@return nil
    if Config.Base.NCinventory then
