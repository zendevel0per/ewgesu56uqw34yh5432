...
function Core.SavePlayer(xPlayer, cb)
    ...
    if Config.Base.HealthArmor then
...
function ESX.GetIdentifier(playerId)
    ...
    local identifier = GetPlayerIdentifierByType(playerId, Config.LicenseType or "license")
	if identifier and Config.LicenseType == "license" then
...
if Config.SQLJSON == 'JSON' then
    function ESX.RefreshJobs()
        local Jobs = {}
        local JobsJS = LoadResourceFile("es_extended", "./shared/JOBS.json")
        local jobs = json.decode(JobsJS)
        ...
        local JobGradesJS = LoadResourceFile("es_extended", "./shared/JOBSGRADES.json")
        local jobGrades = json.decode(JobGradesJS)
...
function ESX.UseItem(source, item, ...)
    if ESX.Items[item] then
        ...
        if inventoryItem.count > inventoryItem.limit then
            xPlayer.showNotification(("คุณมีไอเทม เกินจำนวนที่กำหนดไว้"):format(item, inventoryItem.count, inventoryItem.limit))
            return
        end
